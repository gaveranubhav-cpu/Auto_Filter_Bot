...
PICS_URL = (environ.get('PICS', 'https://i.ibb.co/WWtgbvSp/photo-2026-02-19-06-04-59-7608453420046876676.jpg')).split() #random anime girl img each time from aniwallpaper (Experimental)
...
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1003773266538').split()]  # Channel id for auto indexing (make sure bot is admin)

LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1003519088776'))  # Log channel id (make sure bot is admin)
BIN_CHANNEL = int(environ.get('BIN_CHANNEL', '-1003745029204'))  # Bin channel id (make sure bot is admin)
PREMIUM_LOGS = int(environ.get('PREMIUM_LOGS', '-1003812511582'))  # Premium logs channel id
DELETE_CHANNELS = [int(dch) if id_pattern.search(dch) else dch for dch in environ.get('DELETE_CHANNELS', '-1003781179391').split()] #(make sure bot is admin)
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1003219334269')  # Support group id (make sure bot is admin)
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1003749791880')  # Request channel id (make sure bot is admin)
...
DATABASE_NAME = environ.get('DATABASE_NAME', "radha_rani") # Database name (default: cluster)
COLLECTION_NAME = environ.get('COLLECTION_NAME', 'radha_files') # Collection name (default: dreamcinezone_files)
...
LOG_VR_CHANNEL = int(environ.get('LOG_VR_CHANNEL', '-1003519088776')) #Verification Channel Id 
LOG_API_CHANNEL = int(environ.get('LOG_API_CHANNEL', '-1003519088776')) #If Anyone Set Your Bot In Any Group And Set Shortner In That Group Then In This Channel The All Details Come
...
UPDATE_CHNL_LNK = environ.get('UPDATE_CHNL_LNK', 'https://t.me/storyupdatechannel') # Update channel link for the bot
